...
/* Field gesture boost v16: amplify completed native flicks without blocking iOS scrolling. */
(()=>{
 let start=null;
 document.addEventListener('touchstart',e=>{
   if(e.touches.length!==1)return;
   const t=e.touches[0];
   const carousel=e.target.closest?.('.ghoul-case-carousel');
   const card=e.target.closest?.('.field-card');
   if(!carousel&&!card)return;
   start={x:t.clientX,y:t.clientY,carousel,card};
 },{passive:true});
 document.addEventListener('touchend',e=>{
   ...
   if(Math.abs(dx)>Math.abs(dy)&&start.carousel&&Math.abs(dx)>12){
     const extra=Math.max(-320,Math.min(320,-dx*1.15));
     start.carousel.scrollBy({left:extra,behavior:'smooth'});
   }else if(start.card&&Math.abs(dy)>12){
     const extra=Math.max(-520,Math.min(520,-dy*.9));
     start.card.scrollBy({top:extra,behavior:'smooth'});
   }
   start=null;
 },{passive:true});
