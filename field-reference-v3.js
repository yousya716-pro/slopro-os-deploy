...
/* Field gesture boost v17: bounded horizontal boost; vertical stays native to avoid iOS double-momentum jumps. */
(()=>{
 let start=null;
 document.addEventListener('touchstart',e=>{
   ...
   if(!carousel)return;
   const t=e.touches[0]; start={x:t.clientX,y:t.clientY,carousel};
 },{passive:true});
 document.addEventListener('touchend',e=>{
   if(!start||!e.changedTouches.length){start=null;return;}
   const t=e.changedTouches[0],dx=t.clientX-start.x,dy=t.clientY-start.y,c=start.carousel;
   if(Math.abs(dx)>Math.abs(dy)*1.2&&Math.abs(dx)>16){
     const max=Math.max(0,c.scrollWidth-c.clientWidth);
     const extra=Math.max(-180,Math.min(180,-dx*.65));
     const target=Math.max(0,Math.min(max,c.scrollLeft+extra));
