...
/* Carousel guard v20: after native momentum settles, force the carousel onto the nearest real card start. */
(()=>{
 let timer=0, correcting=false;
 document.addEventListener('scroll',e=>{
   const c=e.target;
   if(!(c instanceof Element)||!c.matches('.ghoul-case-carousel')||correcting)return;
   clearTimeout(timer);
   timer=setTimeout(()=>{
     const cards=[...c.querySelectorAll(':scope > .ghoul-case')];
     if(!cards.length)return;
     const max=Math.max(0,c.scrollWidth-c.clientWidth);
     const points=cards.map(card=>Math.max(0,Math.min(max,card.offsetLeft-c.offsetLeft)));
     const target=points.reduce((best,p)=>Math.abs(p-c.scrollLeft)<Math.abs(best-c.scrollLeft)?p:best,points[0]);
     if(Math.abs(target-c.scrollLeft)>2){correcting=true;c.scrollTo({left:target,behavior:'auto'});requestAnimationFrame(()=>{correcting=false});}
   },140);
 },true);
})();
